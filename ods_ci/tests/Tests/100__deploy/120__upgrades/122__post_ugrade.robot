...
Verify Ray Cluster Exists And Monitor Workload Metrics By Submitting Ray Job After Upgrade
    ...
    Check Requested Resources    ${PRJ_UPGRADE}    ${CPU_SHARED_QUOTA}    ${MEMEORY_SHARED_QUOTA}    ${cpu_requested}    ${memory_requested}    RayCluster

    Check Distributed Workload Resource Metrics Status    ${JOB_NAME}    Running
    Check Distributed Worklaod Status Overview    ${JOB_NAME}    Running    All pods were ready or succeeded since the workload admission
    ...
    [Teardown]    Codeflare Upgrade Tests Teardown    ${PRJ_UPGRADE}    ${PROJECT_CREATED}
